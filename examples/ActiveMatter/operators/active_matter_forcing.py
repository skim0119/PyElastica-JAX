r"""Gravity and traveling-wave actuation for the active-matter snake-pit case.

Each rod is driven by a traveling-wave internal torque field whose arclength
envelope is a cubic spline sampled at the element centers. The torque acts along
the world gravity axis, is rotated into each element material frame, and is
applied as an equal-and-opposite couple across neighboring elements:
...
where ``A(s)`` is the spline envelope, ``T`` the actuation ``time_period``,
``lambda`` the ``wave_length``, and ``r(t)`` the temporal ramp. The sinusoidal
ramp turns on smoothly after ``activation_start_time_nd * T``; the linear ramp
grows over the first period. Gravity is applied as a nodal force along the same
axis.
"""
...
class ActiveMatterForcingJax(eaj.NoBlockOpJax):
    """Nodal gravity and traveling-wave torque actuation on the rod block.

    Parameters
    ----------
    parameters
        Case configuration exposing ``gravitational_acc``, ``time_period``,
        ``wave_length``, and ``activation_start_time_nd``.
    n_snakes : int
        Number of rods packed into the block.
    n_elements : int
        Elements per rod.
    gravity_axis : numpy.ndarray
        Unit axis (shape ``(3,)``) for gravity and the actuation torque.
    spline_amplitude : numpy.ndarray
        Per-element actuation envelope, shape ``(n_elements,)``.
    ramp : str
        Temporal ramp mode, ``"sinusoidal"`` or ``"linear"``.
    """
    ...
    def jax_block_operate_synchronize(self, state, time):
        external_forces, external_torques = _gravity_and_actuation(
            state=state,
...
def _gravity_and_actuation(
    *,
    ...
    """Compute nodal gravity force and traveling-wave torque couple on the block."""
